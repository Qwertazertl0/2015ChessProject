 ...
 * This: PlayerWhite.java
 * Date: November 1, 2015
 * Author: Maxwell Jong
 * Purpose: This class creates a player object who plays the black pieces in the
chess game. It is essentially identical to PlayerBlack.java. The choice to make
...
public class PlayerWhite {
     ...
     * for the PlayerWhite side.
     */
    public PlayerWhite() {
        for (int i = 0; i < 8; i++) {
            pieces[i] = new Pawn(i + 1, 7, true);
        }
        pieces[8] = new Rook(1, 8, true);
        pieces[9] = new Rook(8, 8, true);
        pieces[10] = new Knight(2, 8, true);
        pieces[11] = new Knight(7, 8, true);
        pieces[12] = new Bishop(3, 8, true);
        pieces[13] = new Bishop(6, 8, true);
        pieces[14] = new Queen(4, 8, true);
        king = new King(5, 8, true);
        pieces[15] = king;
    }        
    ...
    /**
     * Checks to see if the current position is a win for PlayerWhite or a stalemate
     */
    public void checkForWin() {
        if (Main.getPlayGraphics() == 0) {
            if (GUIMain.getBlack().hasNoLegalMoves()) {
                if (GUIMain.getBlack().getKing().isInCheck()) {
            ...
            if (TextUIMain.getBlack().hasNoLegalMoves()) {
                if (TextUIMain.getBlack().getKing().isInCheck())
                    JOptionPane.showMessageDialog(null, "Checkmate!\nWhite wins!");
                else JOptionPane.showMessageDialog(null, "It's a stalemate!");
                Computer.setComputerRun(false);
